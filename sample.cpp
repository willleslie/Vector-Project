...
const char *WINDOWTITLE = { "CS553 Project 6 -- William Leslie" };
...
float* rainbow(float value, float min, float max){
	float hsv[3], rgb[3];
	hsv[0] = 240. - 240.*(value - min) / (max - min);
	hsv[1] = 1.;
	hsv[2] = 1.;
	HsvRgb(hsv, rgb);
	return rgb;
...
Advect(float *x, float *y, float *z)
{
	...
	float xb = xa + TimeStep*vxa;
	float yb = ya+ TimeStep*vya;
	float zb = za + TimeStep*vza;
	Vector(xb, yb, zb, &vxb, &vyb, &vzb);
	float vx = (vxa + vxb) / 2.;
	float vy = (vya + vyb) / 2.;
	float vz = (vza + vzb) / 2.;
	float xc = xa + TimeStep*vx;
	float yc = ya + TimeStep*vy;
	float zc = za + TimeStep*vz;
	*x = xc; *y = yc; *z = zc;
}
...
Streamline(float x, float y, float z)
{
	...
	for (int i = 0; i < MAX_ITERATIONS; i++)
	{
		...
		if (sqrt(vx*vx+vy*vy+vz*vz) < SOME_TOLERANCE) break;
		Advect(&x, &y, &z);
	}
	glEnd();
}
...
std::vector<vec3>
Streamline2(float x, float y, float z)
{
...
Display( void )
{
	...
	if (trace == 2){
		glPointSize(5);
		glBegin(GL_POINTS);
			glVertex3f(Xvalue, Yvalue, Zvalue);
		glEnd();
		glPointSize(1);
		Streamline(Xvalue, Yvalue, Zvalue);
	...
	if (trace == 3){
		glBegin(GL_LINE_STRIP);
		glVertex3f(Xvalue, Yvalue, Zvalue);
		glVertex3f(Xvalue - .05, Yvalue, Zvalue);
		glEnd();
		std::vector<vec3> stream1 = Streamline2(Xvalue, Yvalue, Zvalue);
		std::vector<vec3> stream0 = Streamline2(Xvalue-.05, Yvalue, Zvalue);
		std::vector<vec3> stream2 = Streamline2(Xvalue+.05, Yvalue, Zvalue);
		glBegin(GL_QUAD_STRIP);
		for (int i = 0; i < stream1.size(); i++){
			if (i >= stream0.size()) break;
			glVertex3f(stream1.at(i).x, stream1.at(i).y, stream1.at(i).z);
			glVertex3f(stream0.at(i).x, stream0.at(i).y, stream0.at(i).z);
		}
		glEnd();
		/*
		glBegin(GL_QUAD_STRIP);
		for (int i = 0; i < stream1.size(); i++){
			if (i >= stream2.size()) break;
			glVertex3f(stream1.at(i).x, stream1.at(i).y, stream1.at(i).z);
			glVertex3f(stream2.at(i).x, stream2.at(i).y, stream2.at(i).z);
		}
		glEnd();
		*/
	}
...
InitGlui( void )
{
	...
	panel = Glui->add_panel("Trace Types");
		group2 = Glui->add_radiogroup_to_panel(panel, &trace);
			Glui->add_radiobutton_to_group(group2, "Arrows");
			Glui->add_radiobutton_to_group(group2, "Streamlines");
			Glui->add_radiobutton_to_group(group2, "Probe Ribbon");
			Glui->add_radiobutton_to_group(group2, "Probe Streamline");
			Glui->add_radiobutton_to_group(group2, "Blob");
	...
	panel = Glui->add_panel( "Object Transformation" );
		...
		XSpinner = Glui->add_spinner("Probe X",GLUI_SPINNER_FLOAT,&Xvalue, 0);
		XSpinner->set_float_limits(-1, 1);
		YSpinner = Glui->add_spinner("Probe Y", GLUI_SPINNER_FLOAT, &Yvalue, 0);
		YSpinner->set_float_limits(-1, 1);
		ZSpinner = Glui->add_spinner("Probe Z", GLUI_SPINNER_FLOAT, &Zvalue, 0);
		ZSpinner->set_float_limits(-1, 1);
...
float calcSpeed(node thing){
	return sqrt(abs(thing.vx*thing.vx) + abs(thing.vy*thing.vy) + abs(thing.vz*thing.vz));
}
		...
		for (int j = 0; j < NY; j++)
		{
			float y = -1. + 2. * (float)j / (float)(NY - 1);
				for (int k = 0; k < NZ; k++)
				{
					...
					Nodes[i][j][k].y = y;
					Nodes[i][j][k].z = z;
					Vector(x, y, z, &(Nodes[i][j][k].vx), &(Nodes[i][j][k].vy), &(Nodes[i][j][k].vz));
					float mag = calcSpeed(Nodes[i][j][k]);
					if (mag <= 0) printf("%f,%f,%f\n", Nodes[i][j][k].vx, Nodes[i][j][k].vy, Nodes[i][j][k].vz);
					Nodes[i][j][k].mag = mag;
...
InitLists( void )
{
	...
	for (int i = 0; i < NX; i++){
		for (int j = 0; j < NY; j++){
			for (int k = 0; k < NZ; k++){
				rgb = rainbow(Nodes[i][j][k].mag, minvel, maxvel);
				glColor3f(rgb[0], rgb[1], rgb[2]);
				//glScalef(.05, .05, .05);
				tail[0] = Nodes[i][j][k].x - Nodes[i][j][k].vx/(NX*2);
				tail[1] = Nodes[i][j][k].y - Nodes[i][j][k].vy/(NY*2);
				tail[2] = Nodes[i][j][k].z - Nodes[i][j][k].vz/(NZ*2);
				head[0] = Nodes[i][j][k].x + Nodes[i][j][k].vx / (NX * 2);
				head[1] = Nodes[i][j][k].y + Nodes[i][j][k].vy / (NY * 2);
				head[2] = Nodes[i][j][k].z + Nodes[i][j][k].vz / (NZ * 2);
				Arrow(tail, head);
			}
		}
	}
	...
	glNewList(StreamList, GL_COMPILE);
	for (int i = 0; i < NX; i++){
		for (int j = 0; j < NY; j++){
			for (int k = 0; k < NZ; k++){
				Streamline(Nodes[i][j][k].x, Nodes[i][j][k].y, Nodes[i][j][k].z );
			}
		}
	}
